...
| Side chat      | A thread forked from another (`forkedFromThreadId`) on a provider-native fork; hidden from lists and shown beside its parent until `sideChatPromotedAt` is set. See `packages/client-runtime/src/state/sideChat.ts`. |
