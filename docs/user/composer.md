...
On web and desktop, the input stays compact on new and existing threads, including at phone-sized
window widths. Focusing, typing, scrolling, and answering a question do not switch it to an expanded
layout. Longer text grows within the editor's height limit. Model and mode controls appear in the
thread-context strip, with additional controls in its overflow menu.

Questions and task drawers appear above the input. Image, video, and file attachments remain
accessible without changing the editor layout. The phone-sized collapsed row shows up to three
image previews and a count for additional images; tap it to edit the draft and manage attachments.
