...
pending_approval_requests AS (
  ...
  // settled and would otherwise read as "still running" forever (review
  // finding measured on real threads). Ties on equal keys prefer the settled
  // vote. Members of a live workflow ride along through parentAgentId since
  // their lifecycle is progress-only. Newest live tasks first, capped.
  ...
  // Pinning only pays off on root reads of a thread whose provider session can
  ...
  const shouldPinLiveTasks = (
    threadId: ThreadId,
    agentScope: ThreadDetailAgentScope | undefined,
  ) =>
    agentScope !== undefined && threadDetailScopeAgentId(agentScope) !== null
  ...
  const listProjectedThreadActivities = Effect.fn(
    ...
    const [activityIdRows, pinnedActivityIdRows] = yield* Effect.all([
      ...
      shouldPinLiveTasks(threadId, agentScope)
        .pipe(
          Effect.flatMap((includeLiveTasks) =>
            listPinnedThreadActivityIdsByThread({ threadId, includeLiveTasks }),
          ),
        )
        .pipe(
          Effect.mapError(
          ...
          : Effect.all([
              ...
              activityRead.query?.activityKinds === undefined
                ? shouldPinLiveTasks(threadId, agentScope)
                    .pipe(
                      Effect.flatMap((includeLiveTasks) =>
                        listPinnedThreadActivityRowsByThread({ threadId, includeLiveTasks }),
                      ),
                    )
                    .pipe(
                      Effect.mapError(
