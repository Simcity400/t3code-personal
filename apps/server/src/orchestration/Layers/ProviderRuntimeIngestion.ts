          ...
          switch (event.type) {
            ...
            case "turn.completed":
              return normalizeRuntimeTurnState(event.payload.state) === "failed"
                ...
                : "ready";
            case "turn.aborted":
              // An interrupted turn is over. Some providers (OpenCode) send
              // only this event after a stop, never a turn.completed, so the
              // session must settle here or the thread stays "running" until
              // the next message.
              return "ready";
            case "session.started":
            case "thread.started":
