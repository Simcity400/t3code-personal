...
it.layer(NodeServices.layer)("CodexHomeLayout", (it) => {
  ...
  describe("materializeCodexShadowHome", () => {
    ...
    it.effect("preserves existing shadow-local SQLite databases and journals", () =>
      Effect.gen(function* () {
        const fileSystem = yield* FileSystem.FileSystem;
        const path = yield* Path.Path;
        const sharedHome = yield* makeTempDir("t3code-codex-shared-");
    ...
    it.effect("does not link SQLite databases or journals into a new shadow home", () =>
      Effect.gen(function* () {
        const fileSystem = yield* FileSystem.FileSystem;
        const path = yield* Path.Path;
        const sharedHome = yield* makeTempDir("t3code-codex-shared-");
        ...
        for (const suffix of ["", ...runtimeSuffixes]) {
          expect(yield* fileSystem.exists(path.join(shadowHome, `${databaseName}${suffix}`))).toBe(
            false,
          );
        }
      }),
    ...
    it.effect("leaves existing SQLite links and their journals untouched", () =>
      Effect.gen(function* () {
        const fileSystem = yield* FileSystem.FileSystem;
        const path = yield* Path.Path;
        const sharedHome = yield* makeTempDir("t3code-codex-shared-");
        ...
        expect(yield* fileSystem.readFileString(path.join(shadowHome, `${databaseName}-wal`))).toBe(
          "active journal\n",
        );
      }),
    );
