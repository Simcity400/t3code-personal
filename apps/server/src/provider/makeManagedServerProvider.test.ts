...
describe("makeManagedServerProvider", () => {
  it.effect("shares overlapping startup and manual probes without caching completed checks", () =>
    Effect.scoped(
      Effect.gen(function* () {
        ...
        const cancelledWaiter = yield* provider.refresh.pipe(Effect.forkChild);
        yield* Effect.yieldNow;
        yield* Fiber.interrupt(cancelledWaiter);
        const waiters = yield* Effect.all([provider.refresh, provider.refresh, provider.refresh], {
          concurrency: "unbounded",
        }).pipe(Effect.forkChild);
        yield* Effect.yieldNow;
        yield* Deferred.succeed(release, undefined);
        assert.deepStrictEqual(yield* Fiber.join(waiters), [
          refreshedSnapshot,
          refreshedSnapshot,
          refreshedSnapshot,
        ]);
  ...
  it.effect("keeps a verified account and inventory through failed checks, then recovers", () =>
    Effect.scoped(
      Effect.gen(function* () {
        const probe = yield* Ref.make<ServerProvider>(enrichedSnapshot);
        const provider = yield* makeManagedServerProvider<TestSettings>({
          resolveMaintenance: () => Effect.succeed(maintenanceCapabilities),
          getSettings: Effect.succeed({ enabled: true }),
          streamSettings: Stream.empty,
        ...
        for (let attempt = 0; attempt < 2; attempt++) {
          ...
          assert.include(snapshot.message, "Timed out");
        }
        yield* Ref.set(probe, refreshedSnapshotSecond);
        assert.deepStrictEqual(yield* provider.refresh, refreshedSnapshotSecond);
      }),
    ).pipe(Effect.provide(AlwaysRunTestLayer)),
  );

  for (const [name, overrides] of [
    ["sign-out", { auth: { status: "unauthenticated" } }],
    ["missing installation", { installed: false }],
    ["disabled instance", { enabled: false }],
    ["unavailable driver", { availability: "unavailable" }],
    ["different account instance", { instanceId: ProviderInstanceId.make("other") }],
  ] as const) {
    it.effect(`does not retain verified status after ${name}`, () =>
      Effect.scoped(
        Effect.gen(function* () {
          ...
          const provider = yield* makeManagedServerProvider<TestSettings>({
            ...
            checkProvider: Effect.succeed(next),
            refreshInterval: 0,
          });
          assert.deepStrictEqual(yield* provider.refresh, next);
        }),
      ).pipe(Effect.provide(AlwaysRunTestLayer)),
    );
  }

  it.effect("does not carry verified authentication across settings changes", () =>
    Effect.scoped(
      Effect.gen(function* () {
        const settings = yield* Ref.make({ enabled: true, account: "first" });
        const probe = yield* Ref.make<ServerProvider>(enrichedSnapshot);
        ...
        yield* Ref.set(settings, { enabled: true, account: "second" });
        yield* Ref.set(probe, failed);
        assert.deepStrictEqual(yield* provider.refresh, failed);
      }),
    ).pipe(Effect.provide(AlwaysRunTestLayer)),
  );
