...
export function setAgentAwarenessRelayTokenProvider(
  ...
  // The personal build delivers Expo push alerts only; the relay could never
  // update or end its cards, so it does not register there.
  if (!canRegisterPushNotifications() || usesPersonalExpoPushAlerts()) {
